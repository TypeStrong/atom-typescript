...
var TypeScriptProject = (function () {
    ...
    TypeScriptProject.prototype.update = function (config) {
        ...
        var pojectSources = this.projectFilesSet.values.filter(function (fileName) { return _this.isProjectSourceFile(fileName); });
        this.config = config;
        return this.queue.then(function () {
            ...
            return Promise.all(promises).then(function () { return _this.collectFiles(); }).then(function () { return _this.updateWorkingSet(); });
        });
    };
    ...
    TypeScriptProject.prototype.updateWorkingSet = function () {
        var _this = this;
        this.workingSet.getFiles().then(function (files) { return files.forEach(function (fileName) {
            if (_this.projectFilesSet.has(fileName)) {
                _this.languageServiceHost.setScriptIsOpen(fileName, true);
            }
        }); });
    ...
    TypeScriptProject.prototype.isProjectSourceFile = function (fileName) {
        ...
        return this.config.sources.some(function (pattern) { return minimatch(relativeFileName, pattern) || minimatch(fileName, pattern); });
    };
    ...
    TypeScriptProject.prototype.updateReferences = function (fileName, oldFileReferences) {
        ...
        oldFileReferences.values.forEach(function (referencedPath) { return _this.removeReference(fileName, referencedPath); });
    };
