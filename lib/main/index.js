...
function activate(state) {
    ...
    atom.commands.add('atom-workspace', 'typescript:format-code', function (e) {
        ...
        if (selection.isEmpty()) {
            ...
            var formatted = program.formatDocumentRange(filePath, program.languageServiceHost.getIndexFromPosition(filePath, { line: selection.start.row, ch: selection.start.column }), program.languageServiceHost.getIndexFromPosition(filePath, { line: selection.end.row, ch: selection.end.column }));
            editor.setText(program.formatDocument(filePath));
        }
    });
}
