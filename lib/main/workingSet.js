...
var WorkingSet = (function () {
    function WorkingSet(documentManager, editorManager) {
        ...
        this.documentChangesHandler = function (event, document, changes) {
            var changeList = changes.map(function (change) { return ({
                from: change.from,
                to: change.to,
        ...
        this.setFiles(documentManager.getWorkingSet().map(function (file) { return file.fullPath; }));
    ...
    WorkingSet.prototype.setFiles = function (files) {
        var _this = this;
        this.filesSet.values.forEach(function (path) { return _this.filesSet.remove(path); });
        if (files) {
            files.forEach(function (path) { return _this.filesSet.add(path); });
        }
    };
