...
function watchProjectFileIfNotDoingItAlready(projectFile) {
    if (watchingProjectFile[projectFile.projectFilePath])
        return;
    watchingProjectFile[projectFile.projectFilePath] = true;
    fs.watch(projectFile.projectFilePath, { persistent: false, recursive: false }, function () {
        if (!fs.existsSync(projectFile.projectFilePath))
            return;
