...
class ImportManager {
    ...
    addImport(editor, symbol) {
        ...
        if (!existingSource) {
            ...
            const row = existingImports.length === 0 ? 0 : existingImports[existingImports.length - 1].range.end.row + 1;
            editor.getBuffer().insert(new Atom.Point(row, 0), this.getImportString([symbol], importSource, '\n'));
            return;
        }
