...
class TypescriptBuffer {
    ...
    async getErrProject() {
        if (!this.state || !this.state.filePath)
            return;
        await checkAllFiles_1.handleCheckAllFilesResult(this.state.filePath, this.state.filesNames, this.getConfigFilePath(), this.state.client, this.deps.reportProgress);
    }
    /** Throws! */
    async compile() {
        if (!this.state)
